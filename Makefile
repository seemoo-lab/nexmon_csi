...
gen/ucode.asm: $(FW_PATH)/ucode.bin
	...
	$(Q)$(NEXMON_ROOT)/buildtools/b43/disassembler/b43-dasm $< $@ --arch 15 --format raw-le32 2>log/disass.log
...
ifneq ($(wildcard $(NEXMON_ROOT)/buildtools/b43/assembler/b43-asm.bin), )
	$(Q)PATH=$(PATH):$(NEXMON_ROOT)/buildtools/b43/assembler $(NEXMON_ROOT)/buildtools/b43/assembler/b43-asm $< $@ --cpp-args -DRXE_RXHDR_LEN=$(RXE_RXHDR_LEN) -- --format raw-le32
else
...
gen/ucode.asm:
ifneq ($(UCODEFILE),)
	@printf "\033[0;31m  WARNING\033[0m uncompatible ucodefile \"%s\" selected, use \"make UCODEFILE=<filename>\" where filename one of: %s\n" $(UCODEFILE) $(UCODES)
else
